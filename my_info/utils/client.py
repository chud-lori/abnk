...
import requests
from my_info.utils import settings
...
class MyInfoPersonalClientV4(MyInfoClient):
    ...
    context = "com"
    version = "v4"
    client_id = MYINFO_CONNECTOR_CONFIG.get("CLIENT_ID")
    purpose_id = MYINFO_CONNECTOR_CONFIG.get("PURPOSE_ID") # Identity verification and credit assessment
    ...
    @classmethod
    def get_scope(cls):
        return MYINFO_CONNECTOR_CONFIG.get("SCOPE")
    ...
    def get_person_data(self, access_token: str, session_ephemeral_keypair):
        jwkset = get_jwkset(MYINFO_CONNECTOR_CONFIG.get("AUTHORIZE_JWKS_URL"))
